...
def sample(model, sample_batch_size, obs, sample_op, conditional= False, labels=None):
    model.train(False)
    with torch.no_grad():
        ...
        for i in range(obs[1]):
            for j in range(obs[2]):
                data_v = data                
                out   = model(data_v, labels=labels, sample=True) if conditional else model(data_v, sample=True)
