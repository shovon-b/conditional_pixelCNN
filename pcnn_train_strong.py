...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--batch_size', type=int, default=64, help='Total batch size during training (will be split across GPUs)')
    parser.add_argument('-x', '--max_epochs', type=int, default=5000, help='How many epochs to run in total?')
    parser.add_argument('-s', '--seed', type=int, default=1, help='Random seed to use')
    parser.add_argument('--conditional', type=bool, default=False, help='Enable conditional generation based on class labels')
    parser.add_argument('--num_class', type=int, default=4, help='Number of classes for conditional generation')
    ...
    if args.load_params:
        # Load state dict into the unwrapped model if using DataParallel
    ...
    for epoch in tqdm(range(args.max_epochs)):
        ...
        if (epoch + 1) % args.save_interval == 0: 
            ...
            # Save the unwrapped model state if using DataParallel
            state_dict = model.module.state_dict() if isinstance(model, nn.DataParallel) else model.state_dict()
            torch.save(state_dict, os.path.join(args.save_dir, f'{model_name}_{epoch}.pth'))