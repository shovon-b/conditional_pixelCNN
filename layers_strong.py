...
#network in network class: changes the channel size: inpt: image[B, dim_in, W,H] -> out: image[B, dim_out, W,H]
class nin(nn.Module):
...
class gated_resnet(nn.Module):
    def __init__(self, num_filters, conv_op, nonlinearity=concat_elu, skip_connection=0, conditional=False, num_class=4):
        ...
        self.conv_input = conv_op(2 * num_filters, num_filters) # *2 because concat elu doubles the channel size
        ...
        if self.conditional:
            self.embedding1 = nn.Embedding(num_class, num_filters)
            self.embedding2 = nn.Embedding(num_class, 2 * num_filters)
        else:
    ...
    def forward(self, og_x, labels=None, a=None):
        if self.conditional:
            ...
            bias1 = self.embedding1(labels).view(-1, self.num_filters, 1, 1)
            bias2 = self.embedding2(labels).view(-1, 2 * self.num_filters, 1, 1)
            #print("conditional model running")
        else:
            bias = None
        x = bias1 + self.conv_input(self.nonlinearity(og_x)) if self.conditional else self.conv_input(self.nonlinearity(og_x))        
        if a is not None :
            x += self.nin_skip(self.nonlinearity(a))                
        x = self.nonlinearity(x)
        x = self.dropout(x)
        x = self.conv_out(x) + bias2 if self.conditional else self.conv_out(x)        
